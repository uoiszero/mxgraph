...
// ensureAdvancedShapeDefs: 注册高级箭头等扩展形状（若外部脚本未加载成功）
function ensureAdvancedShapeDefs() {
    ...
    DoubleArrowShape.prototype.redrawPath = function (c, x, y, w, h) {
      var aw =
        ...
        Math.max(
          ...
          Math.min(
            1,
            parseFloat(mxUtils.getValue(this.style, "arrowWidth", 0.3))
          )
        );
      var as =
        w *
        Math.max(
          ...
          Math.min(
            1,
            parseFloat(mxUtils.getValue(this.style, "arrowSize", 0.2))
          )
        );
      var at = (h - aw) / 2;
      var ab = at + aw;
  ...
  if (
    !window.__mx_custom_handles_installed &&
    window.mxHandle &&
    window.mxEdgeHandler
  ) {
    ...
    mxEdgeHandler.prototype.createCustomHandles = function () {
      ...
      function addWidthHandle() {
        ...
        h.getPosition = function (bounds) {
          ...
          if (!p0 || !p1)
            return new mxPoint(state.getCenterX(), state.getCenterY());
          var cx = (p0.x + p1.x) / 2;
          var cy = (p0.y + p1.y) / 2;
          var dx = p1.x - p0.x;
          var dy = p1.y - p0.y;
          ...
          var nx = -dy / len;
          var ny = dx / len;
          var w = mxUtils.getNumber(
            state.style,
            "width",
            shapeName === "flexArrow" ? 10 : 4
          );
          var off = w / 2;
      ...
      if (shapeName === "link") {
        ...
        for (var j = 0; j < handles.length; j++) {
          if (handles[j] && handles[j].__role !== "curve")
            vendor.push(handles[j]);
        }
  ...
  if (
    !window.__mx_vertex_thickness_handle_installed &&
    window.mxHandle &&
    window.mxVertexHandler
  ) {
    ...
    mxVertexHandler.prototype.createCustomHandles = function () {
      var handles = origVertexHandles
        ? origVertexHandles.apply(this, arguments)
        : null;
      var list = handles || [];
      var st = this.state;
      if (st && this.graph.getModel().isVertex(st.cell)) {
        ...
        hh.getPosition = function (bounds) {
          var sw =
            (st.shape && st.shape.strokewidth) ||
            mxUtils.getNumber(st.style, "strokeWidth", 1);
...
export default {
  ...
  setup(props) {
    ...
    const source = props.stencils.length
      ? props.stencils
      : defaultStencilUrls();
    ...
    function styleForItem(it) {
      let s =
        it.style ||
        (it.shapeKey
          ? "shape=" + it.shapeKey + ";whiteSpace=wrap;html=1"
    ...
    function populateGeneral(items) {
      ...
      const push = (label, w, h, style, value = "") =>
        items.push({ key: "general." + label, label, w, h, style, value });
    ...
    function populateMisc(items) {
      if (!items) return;
      const push = (label, w, h, style, value = "") =>
        items.push({ key: "misc." + label, label, w, h, style, value });
      push(
        "Double Rectangle",
        120,
        80,
    ...
    function populateAdvanced(items) {
      if (!items) return;
      const push = (label, w, h, style, value = "") =>
        items.push({ key: "advanced." + label, label, w, h, style, value });
      push(
        "Tape Data",
        80,
        80,
        ...
        if (node.nodeType === 1 && node.nodeName === "shape") {
          ...
          if (raw) {
            ...
            if (
              ...
              !mxStencilRegistry.stencils[key]
            ) {
              try {
    ...
    function renderThumbDom(item, width, height) {
      ...
      const s = Math.max(
        0.01,
        Math.floor(
          Math.min((width - 4) / bounds.width, (height - 4) / bounds.height) *
            100
        ) / 100
      );
      ...
      let node = null;
      if (
        g.dialect == mxConstants.DIALECT_SVG &&
        !mxClient.NO_FO &&
        g.view.getCanvas().ownerSVGElement
      ) {
