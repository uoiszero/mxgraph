...
export function registerCustomShapes(mxns) {
  ...
  /**
   * 强制重绘逻辑（针对 flexArrow）
   * 当仅样式参数变化（width/startWidth/endWidth/startSize/endSize），但端点/包围盒未变时，默认不重绘；
   * 这里扩展 isShapeInvalid，使此类变化也触发 redraw，避免“调整后消失”现象。
   * @param {any} state
   * @param {any} shape
   * @returns {boolean}
   */
  const origIsShapeInvalid = mxCellRenderer.prototype.isShapeInvalid;
  mxCellRenderer.prototype.isShapeInvalid = function (state, shape) {
    const isFlex = !!(
      shape &&
      shape.style &&
      shape.style["shape"] === "flexArrow"
    );
    const style = state && state.style ? state.style : null;
    const sstyle = shape && shape.style ? shape.style : null;
    const changedWidthLike =
      isFlex &&
      style &&
      sstyle &&
      (mxUtils.getNumber(style, "width", 10) !==
        mxUtils.getNumber(sstyle, "width", 10) ||
        mxUtils.getNumber(style, "startWidth", 20) !==
          mxUtils.getNumber(sstyle, "startWidth", 20) ||
        mxUtils.getNumber(style, "endWidth", 20) !==
          mxUtils.getNumber(sstyle, "endWidth", 20) ||
        mxUtils.getNumber(
          style,
          mxConstants.STYLE_STARTSIZE,
          (mxConstants.ARROW_SIZE / 5) * 3
        ) !==
          mxUtils.getNumber(
            sstyle,
            mxConstants.STYLE_STARTSIZE,
    ...
    return (
      origIsShapeInvalid.apply(this, arguments) ||
      changedWidthLike ||
      routedMismatch
    );
  };
  ...
  TapeShape.prototype.redrawPath = function (c, x, y, w, h) {
    ...
    c.quadTo((3 * w) / 4, dy * (1 - fy), w, dy / 2);
    c.lineTo(w, h - dy / 2);
    c.quadTo((3 * w) / 4, h - dy * fy, w / 2, h - dy / 2);
  ...
  ProcessShape.prototype.paintForeground = function (c, x, y, w, h) {
    ...
    inset = isFixedSize
      ? Math.max(0, Math.min(w, inset))
      : w * Math.max(0, Math.min(1, inset));
    if (this.isRounded) {
      var f =
        mxUtils.getValue(
          this.style,
  ...
  ParallelogramShape.prototype.redrawPath = function (c, x, y, w, h) {
    ...
    this.addPoints(
      ...
      [
        new mxns.mxPoint(0, h),
        new mxns.mxPoint(dx, 0),
        new mxns.mxPoint(w, 0),
        new mxns.mxPoint(w - dx, h)
      ],
      this.isRounded,
      arcSize,
      true
  ...
  SingleArrowShape.prototype.redrawPath = function (c, x, y, w, h) {
    ...
    this.addPoints(
      c,
      [
        new mxns.mxPoint(0, at),
        new mxns.mxPoint(w - as, at),
        new mxns.mxPoint(w - as, 0),
        new mxns.mxPoint(w, h / 2),
        new mxns.mxPoint(w - as, h),
        new mxns.mxPoint(w - as, ab),
        new mxns.mxPoint(0, ab)
      ],
      this.isRounded,
      arcSize,
      true
  ...
  DoubleArrowShape.prototype.redrawPath = function (c, x, y, w, h) {
    ...
    this.addPoints(
      c,
      [
        new mxns.mxPoint(0, h / 2),
        new mxns.mxPoint(as, 0),
        new mxns.mxPoint(as, at),
        new mxns.mxPoint(w - as, at),
        new mxns.mxPoint(w - as, 0),
        new mxns.mxPoint(w, h / 2),
        new mxns.mxPoint(w - as, h),
        new mxns.mxPoint(w - as, ab),
        new mxns.mxPoint(as, ab),
        new mxns.mxPoint(as, h)
      ],
      this.isRounded,
      arcSize,
      true
  ...
  LoopLimitShape.prototype.redrawPath = function (c, x, y, w, h) {
    ...
    this.addPoints(
      ...
      [
        new mxns.mxPoint(s, 0),
        new mxns.mxPoint(w - s, 0),
        new mxns.mxPoint(w, s * 0.8),
        new mxns.mxPoint(w, h),
        new mxns.mxPoint(0, h),
        new mxns.mxPoint(0, s * 0.8)
      ],
      this.isRounded,
      arcSize,
      true
  ...
  CornerShape.prototype.redrawPath = function (c, x, y, w, h) {
    var dx = Math.max(0, Math.min(w, mxUtils.getNumber(this.style, "dx", 20)));
    var dy = Math.max(0, Math.min(h, mxUtils.getNumber(this.style, "dy", 20)));
    var arcSize =
      mxUtils.getValue(
        this.style,
        mxConstants.STYLE_ARCSIZE,
  ...
  UmlControlShape.prototype.paintBackground = function (c, x, y, w, h) {
    c.translate(x, y);
    c.begin();
    c.moveTo((3 * w) / 8, (h / 8) * 1.1);
    c.lineTo((5 * w) / 8, 0);
    c.end();
    c.stroke();
    c.ellipse(0, h / 8, w, (7 * h) / 8);
    c.fillAndStroke();
  };
  ...
  function UmlLifeline() {
    mxns.mxRectangleShape.call(this);
  }
  ...
  UmlLifeline.prototype.paintBackground = function (c, x, y, w, h) {
    ...
    mxns.mxRectangleShape.prototype.paintBackground.call(
      this,
      c,
      x,
      y,
  ...
  UmlLifeline.prototype.paintForeground = function (c, x, y, w, h) {
    ...
    mxns.mxRectangleShape.prototype.paintForeground.call(
      this,
      c,
      x,
      y,
  ...
  TableShape.prototype.paintVertexShape = function (c, x, y, w, h) {
    var start = this.getTitleSize();
    if (start == 0) {
      mxns.mxRectangleShape.prototype.paintBackground.apply(this, arguments);
    } else {
      mxns.mxSwimlane.prototype.paintVertexShape.apply(this, arguments);
      c.translate(-x, -y);
    }
